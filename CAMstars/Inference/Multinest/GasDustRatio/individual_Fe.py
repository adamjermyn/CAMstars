...
# Whitelist of elements in accretingPop
whiteList = ['Fe']
for m in accretingPop.materials:
	nn = list(m.names)
	for e in nn:
...
def infer(s):
	...
	elements = list(e for e in s.names if e in reference.species)
	...
	diff = list(s.query(e)[0] - reference.queryStats(e)[0] for e in elements)
	var = list(s.query(e)[1]**2 + reference.queryStats(e)[1]**2 for e in elements)

	print(s.name, s.query(e)[0], reference.queryStats(e)[0])
	...
	oDir = dir_path + '/../../../../Output/GasDust_Fe_' + s.name + '/'
	...
	refX = np.array([reference.queryStats(e)[0] for e in elements])
	refV = np.array([reference.queryStats(e)[1] for e in elements])
	model = np.array([reference.queryStats(e)[0] + np.log(1-fAcc + fAcc * (1-fX[elements.index(e)] + 10**(logd)*fX[elements.index(e)])) for e in elements])
